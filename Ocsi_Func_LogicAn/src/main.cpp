...
uint32_t logican_sample_us = 105;         // sample time at 9600 bps 8N1
uint8_t decoded_byte  = 00  ;             // decode byte from serial
uint8_t bit_counter   = 00  ;             // counts the number of bits
uint8_t logi_temp_var   = 00  ;           // a temporary variable used in logic analyzer routine
...
void logicanRead(){
    /*------------ To get value from 9600 N 1 serail communication (START) ---------------*/
    ...
    { bool rx_bit = digitalRead(SERIAL_RX_PIN);		/* Keep for check acknowledge	*/
          logi_temp_var =0;	
	        logi_temp_var = logi_temp_var | rx_bit;	/* Keep bit data in dat */
          logi_temp_var = logi_temp_var<<bit_counter;
          decoded_byte = decoded_byte | logi_temp_var;	
          bit_counter ++ ;
    ...
    { Serial.print(decoded_byte) ;
       for (int x= 50; x <= 100; x++){              // clear the last value
            for(int y=20; y<=40; y++){ 
              display.drawPixel(x,y, SSD1306_BLACK);  }
          }
    ...
    /*------------ To get value from 9600 N 1 serail communication (END) ---------------*/

    /*------------ To get Real Time Pin value at Serial Terminal (Start) ---------------*/
    //  Serial.println(digitalRead(SERIAL_RX_PIN)*255);
     // Serial.flush();
     /*-------------To get Real Time Pin value at Serial Terminal (END)----------------*/
}
